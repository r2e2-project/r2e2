...
public:
  ...
  JoinIterator( Iterator t1_begin,
                Iterator t1_end,
                Iterator t2_begin,
                Iterator t2_end,
  ...
  bool operator==( const JoinIterator& other )
  {
    if ( current_ != other.current_
         or iterators_.size() != other.iterators_.size() ) {
      return false;
    }

    for ( size_t i = 0; i < iterators_.size(); i++ ) {
      if ( iterators_[i].first != other.iterators_[i].first
           or iterators_[i].second != other.iterators_[i].second ) {
        return false;
      }
    }
...
public:
  JoinContainer( Iterator c1_begin,
                 Iterator c1_end,
                 Iterator c2_begin,
                 Iterator c2_end )
...
JoinContainer<typename Container::iterator> join_containers( Container& c1,
                                                             Container& c2 )
{
...
template<class Container>
JoinContainer<typename Container::const_iterator> join_containers(
  const Container& c1,
  const Container& c2 )
{
  JoinContainer<typename Container::const_iterator> x(
    c1.cbegin(), c1.cend(), c2.cbegin(), c2.cend() );
  return x;
}
