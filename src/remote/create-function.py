...
def create_function_package(output, r2t2_lambda_worker, storageserver):
    PACKAGE_FILES = {
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate and install Lambda functions."
    )
    ...
    parser.add_argument(
        "--region", dest="region", default=os.environ.get("AWS_REGION"), action="store"
    )
    parser.add_argument(
        "--name",
        dest="name",
        default=os.environ.get("R2T2_LAMBDA_FUNCTION", "r2t2-lambda-function"),
        action="store",
    )
    parser.add_argument(
        "--r2t2-lambda-worker",
        dest="r2t2_lambda_worker",
        default=shutil.which("r2t2-lambda-worker"),
        required=True,
    )
    parser.add_argument("--storageserver", dest="storageserver", required=True)
    ...
    try:
        create_function_package(
            function_file, args.r2t2_lambda_worker, args.storageserver
        )
        print("Installing lambda function {}... ".format(function_name), end="")
        install_lambda_package(
            function_file, function_name, args.role, args.region, delete=args.delete
        )
    finally:
