...
def create_function_package(output, benchmark_worker):
    PACKAGE_FILES = {
        "benchmark-worker": benchmark_worker,
        "main.py": "benchmark-function/main.py",
    }
...
def main():
    ...
    parser.add_argument('--name', dest='name', default="r2t2-s3-benchmark",
                        action='store')
    parser.add_argument('--benchmark-worker', dest='benchmark_worker',
                        default=shutil.which("benchmark-worker"))

    args = parser.parse_args()

    if not args.benchmark_worker:
    ...
    try:
        create_function_package(function_file, args.benchmark_worker)
        print("Installing lambda function {}... ".format(function_name), end='')
        install_lambda_package(function_file, function_name, args.role,
                               args.region, delete=args.delete)
    finally:
