...
Poller::Action::Action(NBSecureSocket& s_socket,
    ...
    if (direction == Out) { /* write */
        callback = [s_callback, &s_socket]() {
            ...
            if (s_socket.mode() == NBSecureSocket::Mode::connect and
                not s_socket.connected()) {
                /* we're not connected yet, so let's continue */
                s_socket.continue_SSL_connect();
            } else if (s_socket.mode() == NBSecureSocket::Mode::accept and
                       not s_socket.accepted()) {
        ...
        when_interested = [s_when_interested, &s_socket]() {
            return (s_socket.state() == NBSecureSocket::State::needs_connect) or
                   (s_socket.state() ==
                    NBSecureSocket::State::needs_ssl_write_to_connect) or
                   (s_socket.state() ==
                    NBSecureSocket::State::needs_ssl_write_to_accept) or
                   (s_socket.state() ==
                    NBSecureSocket::State::needs_ssl_write_to_write) or
                   (s_socket.state() ==
                    NBSecureSocket::State::needs_ssl_write_to_read) or
                   (s_socket.state() == NBSecureSocket::State::ready and
                    s_when_interested());
        };
        ...
        callback = [s_callback, &s_socket]() {
            if (s_socket.mode() == NBSecureSocket::Mode::connect and
                ...
                s_socket.continue_SSL_write();
                           ...
                           NBSecureSocket::State::needs_ssl_read_to_read or
        ...
        when_interested = [s_when_interested, &s_socket]() {
            return (s_socket.state() == NBSecureSocket::State::needs_connect) or
                   ...
                   (s_socket.state() ==
                    NBSecureSocket::State::needs_ssl_read_to_connect) or
                   (s_socket.state() ==
                    NBSecureSocket::State::needs_ssl_read_to_accept) or
                   (s_socket.state() ==
                    NBSecureSocket::State::needs_ssl_read_to_write) or
                   (s_socket.state() ==
                    NBSecureSocket::State::needs_ssl_read_to_read) or
                   (s_socket.state() == NBSecureSocket::State::ready and
                    s_when_interested());
...
void NBSecureSocket::continue_SSL_write() {
    try {
        SecureSocket::write(
            write_buffer_.size() ? write_buffer_.front() : string(),
