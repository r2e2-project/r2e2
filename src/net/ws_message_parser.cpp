    ...
    while (not raw_buffer_.empty()) {
        ...
        case WSFrame::OpCode::Continuation:
            if (frame_buffer_.size() == 0) {
                throw runtime_error(
                    "message cannot start with a continuation frame");
            }
        ...
        case WSFrame::OpCode::Binary:
            if (frame_buffer_.size() != 0) {
                throw runtime_error(
                    "expected a continuation message, got text/binary");
            }
        ...
        case WSFrame::OpCode::Pong:
            ...
            /* we don't put control frames into the frame buffer, we directly
            create the message from those and push them into the output queue */
