...
bool is_valid_handshake_request(const HTTPRequest &request) {
    ...
    if (first_line.substr(last_space + 1) != "HTTP/1.1" and
        ...
        cerr << "Invalid WebSocket request: only allow HTTP/1.1 and HTTP/2"
             << endl;
        return false;
    }

    if (not request.has_header("Connection") or
        request.get_header_value("Connection").find("Upgrade") ==
            string::npos) {
        cerr << "Invalid WebSocket request: 'Connection: Upgrade' is required"
             << endl;
        return false;
    }
    ...
    if (not request.has_header("Upgrade") or
        ...
        cerr << "Invalid WebSocket request: 'Upgrade: websocket' is required"
             << endl;
        return false;
    }
    ...
    if (not request.has_header("Sec-WebSocket-Key")) {
        cerr << "Invalid WebSocket request: 'Sec-WebSocket-Key' is required"
             << endl;
        return false;
    }
...
template <>
WSServer<TCPSocket>::Connection::Connection(TCPSocket &&sock)
    : socket(move(sock)) {}
...
void WSServer<TCPSocket>::Connection::write() {
    if (not send_buffer.empty()) {
        ...
        if (next_it != buffer.cend()) {
            ...
            send_buffer_offset = next_it - buffer.cbegin();
        } else {
                ...
                [this, &conn, conn_id]() -> ResultType {
                    ...
                    if (conn.state == Connection::State::NotConnected) {
                        try {
                            ...
                            /* close the connection if received an invalid
                             * message */
                        ...
                        while (not conn.ws_handshake_parser.empty()) {
                            auto request =
                                move(conn.ws_handshake_parser.front());
                            conn.ws_handshake_parser.pop();

                            const auto &response =
                                create_handshake_response(request);
                            conn.send_buffer.emplace_back(response.str());

                            /* only continue with status code of 101 */
                            if (response.status_code() != "101") {
                                /* TODO: response will not reach the client side
                                 * currently */
                                force_close_connection(conn_id);
                                return ResultType::CancelAll;
                            }
                        ...
                        while (not conn.ws_message_parser.empty()) {
                            WSMessage message =
                                move(conn.ws_message_parser.front());
                            ...
                            case WSMessage::Type::Close: {
                                ...
                                WSFrame close_frame{true,
                                                    WSFrame::OpCode::Close,
                                                    message.payload()};
                        ...
                        cerr << "Invalid conn.state = " << (int)conn.state
                             << endl;
                        force_close_connection(conn_id);
                        return ResultType::CancelAll;
                    }
...
bool WSServer<SocketType>::queue_frame(const uint64_t connection_id,
    ...
    /* frame.to_string() inevitably copies frame.payload_ into the return
     * string,
...
void WSServer<SocketType>::wait_close_connection(const uint64_t connection_id) {
    ...
    if (conn.state != Connection::State::Connected) {
        cerr << "Warning: wait_close_connection is called but not connected"
             << endl;
        return;
    }
...
template <class SocketType>
void WSServer<SocketType>::force_close_connection(
...
void WSServer<SocketType>::clean_idle_connection(const uint64_t connection_id) {
    ...
    poller_.remove_fds({conn.socket.fd_num()});
